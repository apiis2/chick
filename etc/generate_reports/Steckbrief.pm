...
sub sub_get_data {
    ...
    foreach my $record ( @alltables ) {
        ...
        #-- globale Variablen 
        my $type="(select ext_code from codes inner join event on codes.db_code=event.db_event_type where
        ...
        next if ($table eq 'possible_dams');
                ...
                or ( $sql_ref->{ '_rows' } eq '0E0' ) )
        {
            ...
            foreach my $animal ( @animal ) {
                ...
                $hs_animal->{ $animal }->{ 'stamm' }->{ 'Ident' } = [
                                              ...
                                              main::__('Rasse'),         main::__('Abgangsdatum'),  
                ];
...
';
    ...
    foreach my $ani ( sort keys %{ $data } ) {
        ...
        $self->{ '_longtablecontent' }
            .= "\\begin{tabular}{p{25mm}p{50mm}p{30mm}p{55mm}} \n";

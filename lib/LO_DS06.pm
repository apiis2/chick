...
#    };
    ...
    foreach my $breed (sort keys %{ $hs_check }) {
        ...
        foreach my $cage (keys %{ $hs_check->{$breed}->{'cages'} }) {
            ...
            $hs_check->{$breed}->{'far_cage'}->[$pos] =$cage;
        ...
        for (my $i=1;$i<=$#{$hs_check->{$breed}->{'far_dam'}};$i++) {
            ...
            my $cage=$hs_check->{$breed}->{'far_cage'}->[$i];

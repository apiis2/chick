#####################################################################
# load object: LO_DS07
# $Id: LO_DS07.pm,v 1.10 2022/04/01 13:16:31 lfgroene Exp $
#####################################################################
...
#                    $data[13]=$datacsv[12];
    ...
    if (exists $args->{'Fanimal_ext_id'} ) {
        ...
        $sql="select distinct date_part('year',hatch_dt) from hatch_cage a inner join v_active_animals_and_cages b on a.db_cage=b.db_cage;";        
        #-- SQL auslösen 
